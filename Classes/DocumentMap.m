...
-(DocumentNode *)createDocumentNodes
{
	...
	DocumentNodeType currNodeType;
    NSString *currNodeText;
    NSString *currNodeContentFile;
...
//        validNode = NO;
		...
		if ((currNodeType == ROOT)||(currNodeType == HEADER)) {
			...
			currNode = [[DocumentNode alloc]initWithTextAndType:currNodeText nodeType:currNodeType];
		} else if (currNodeType == CONTENT) {
            NSLog(@"Create CONTENT node %@.", currNodeText);
			currNode = [[DocumentNode alloc]initWithTextTypeContentFile:currNodeText nodeType:currNodeType contentFile:currNodeContentFile];
